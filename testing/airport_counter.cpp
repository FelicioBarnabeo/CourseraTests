...
public:
    ...
    inline size_t Get(TAirport airport) const
    {
        return data[(size_t)airport];
    }
    ...
    Items GetItems() const
    {
        ...
        for(int i=0; i<SIZE; ++i){
            items[i] = {(TAirport)i, data[i]};
        }
        return items;
    }
private:
    array<size_t,SIZE> data{0};
...
//int main() {
//  TestRunner tr;
//  //Суммарное время должно быть не более двух секунд, не забудь включить флаги оптимизации
